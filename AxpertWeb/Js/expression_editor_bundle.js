///////////////////////////////////////////////////////////////////
//@MANIKANTA                                                     //
//  There are only two days in the yeat notthing can be done     //
//  One is called yesterday and the other is called Tomorrow     //
//  Today is the right day to  Love, Believe, Do and mostly Live //
///////////////////////////////////////////////////////////////////

/**
 * For Expression Editor following files are required in order to load dynamically
 * 1 -> codemirror/codemirror.js
 * 2 -> codemirror/addon/hint/show-hint.js
 * 3 -> codemirror/addon/codemirror_grammar.min.js
 * 4 -> expression_editor.js
 */
...
//~~~~~~~~~codemirror/addon/codemirror_grammar.min.js~~~~~~~~~START~~~~~~~~~~~~~~
 ...
 *
 **/
if (typeof CodeMirrorGrammar === "undefined") {
    ! function(root, name, factory) {
        ...
        "CodeMirrorGrammar",
        /* module factory */
        function ModuleFactory__CodeMirrorGrammar() {
            ...
            var
                ...
                T_ONEORMORE = 8195,
                T_LOOKAHEAD = 16384,
                T_POSITIVE_LOOKAHEAD = T_LOOKAHEAD,
                T_NEGATIVE_LOOKAHEAD = 16385,
                T_NGRAM = 32768,
                T_SUBGRAMMAR = 65536,
                ...
                tokenTypes = {
                    ...
                    positivelookahead: T_POSITIVE_LOOKAHEAD,
                    negativelookahead: T_NEGATIVE_LOOKAHEAD,
                    ngram: T_NGRAM
                },
            ...
            var undef = undefined,
                ...
                IS_ENUM = OP.propertyIsEnumerable,
            ...
            function operate(x, F, F0, i0, i1) {
                ...
                for (i = r; i < l; i += 16) {
                    k = i0 + i;
                    Fv = F(F(F(F(F(F(F(F(F(F(F(F(F(F(F(F(Fv, x[k], k), x[k + 1], k + 1), x[k + 2], k + 2), x[k + 3], k + 3), x[k + 4], k + 4), x[k + 5], k + 5), x[k + 6], k + 6), x[k + 7], k + 7), x[k + 8], k + 8), x[k + 9], k + 9), x[k + 10], k + 10), x[k + 11], k + 11), x[k + 12], k + 12), x[k + 13], k + 13), x[k + 14], k + 14), x[k + 15], k + 15);
                }
                return Fv;
            }
            ...
            */
            ...
            function group_replace(pattern, token, raw, in_regex) {
                var i, l, c, g, replaced, offset = true === raw ? 0 : 1,
                    placeholder = in_regex ? '\\' : '$',
                    placeholder_code = in_regex ? 92 : 36;
            ...
            function get_autocomplete(tok, autocompletions, type, keywords) {
                var meta = tok.meta || type,
                    case_insesitive = !!(tok.caseInsesitive || tok.ci),
                ...
                for (id in G) {
                    ...
                    if (T_OBJ === get_type(tok) && !tok.type) {
                        if (tok['ngram'] || tok['n-gram']) {
                            ...
                            if (tok['lookahead']) del(tok, 'lookahead');
                            else del(tok, 'positiveLookahead');
                            ...
                            if (tok['subgrammar']) del(tok, 'subgrammar');
                            else del(tok, 'grammar');
                        }
                    } else if (tok.type) {
                        tl = tok.type = tok.type[LOWER]();
                ...
                return expression.length > 1 ?
                    iterate(is_alternation_else_sequence
                        ...
                        function(i, simplified) {
                            var current = simplified[simplified.length - 1],
                                next = expression[i];
                            if (current === next) { /* skip*/ } else simplified.push(next);
                        }
                        ...
                        function(i, simplified) {
                            ...
                            /*else*/
                            if (trailing_repeat_re.test(next) && trailing_repeat_re.test(current) && current === next) { /* skip*/ } else simplified.push(next);
                    ...
                    while (t.pos < t.length) {
                        ...
                        if (peg_bnf_special_re.test(c)) {
                            ...
                            if ('.' === c /*|| ':' === c*/ ) {
                                ...
                                /*if ( !token.length )
                                {*/
                                literal = get_delimited(t, c, '\\', 0);
                                fl = '';
                                if (literal.length) {
                                    if (t.pos < t.length && 'i' === t[CHAR](t.pos)) {
                                        t.pos++;
                                        fl = 'i';
                                    }
            ...
            function get_tokenizer(tokenID, RegExpID, Lex, Syntax, Style,
                ...
                var $token$ = null,
                    $msg$ = null,
                    $modifier$ = null,
                    $type$, $tokens$, t, tt, token, combine, autocompletions;
                ...
                if (T_ACTION & $type$) {
                    ...
                    if (T_SIMPLE & $type$) {
                        ...
                        $token$ = new tokenizer(T_SIMPLE, tokenID,
                            get_compositematcher(tokenID, $tokens$.slice(), RegExpID, combine, !!(token.caseInsensitive || token.ci), cachedRegexes, cachedMatchers, kws),
                        ...
                        if (null != token.except) {
                            var token_except = make_array(token.except),
                                i, l = token_except.length,
                                except = [],
                                tok_exce;
                            for (i = 0; i < l; i++) {
                                if (!!token_except[i]) {
                        ...
                        if (T_SUBGRAMMAR === $type$) {
                            ...
                            if (T_POSITIVE_LOOKAHEAD === $type$ || T_NEGATIVE_LOOKAHEAD === $type$) {
                                ...
                                $token$.min = token.repeat[0];
                                $token$.max = token.repeat[1];
            ...
            function t_match(t, stream, eat, any_match) {
                ...
                if (P_BLOCK === PT) {
                    ...
                    if (match = t_match(start, stream, eat, any_match)) {
                        ...
                        T = get_type(end);
                        T0 = start.pattern[match[0]].ptype;
            ...
            function stack_clone(stack, deep) {
                ...
                if (deep) {
                    var stack2 = new Stack(stack.val),
                        ptr2 = stack2,
                        ptr = stack;
                    while (ptr.prev) {
            ...
            function err_recover(state, stream, token, tokenizer) {
                ...
                var stack_pos, stream_pos, stream_pos2, tok, depth,
                    recover_stream = Infinity,
                    recover_stack = null,
                    recover_depth = Infinity;
            ...
            function t_action(a, stream, state, token) {
                var self = a,
                    action_def = self.token || null,
                    action, case_insensitive = self.ci,
                    aid = self.name,
                ...
                action = action_def[0];
                t = action_def[1];
                options = action_def[2] || {};
                msg = self.msg;
                in_ctx = options['in-context'];
                in_hctx = options['in-hypercontext'];

                if (is_block /*&& token.block*/ ) {
                    t_str = token.block.match || token.block.str;
                    l1 = token.block.pos[0][0];
                    c1 = token.block.pos[0][1];
                    l2 = token.block.pos[0][2];
                    c2 = token.block.pos[0][3];
                    raw = true;
                } else {
            ...
            function t_simple(t, stream, state, token, exception) {
                ...
                if (except && !exception) {
                    ...
                    for (var i = 0, l = except.length; i < l; i++) {
                        ...
                        if (t_simple(tok_except, stream, state, token, 1)) {
                            self.pos = tok_except.pos;
            ...
            function t_block(t, stream, state, token) {
                var self = t,
                    ...
                    type = self.type,
                    modifier = self.modifier,
                    block_start = self.token,
                    block_end,
                    is_multiline = self.mline,
                    has_interior = self.inter,
                    ...
                    outer = state.outer,
                    outerState = outer && outer[2],
                    outerTokenizer = outer && outer[1];
                ...
                */
                ...
                is_required = self.status & REQUIRED;
                already_inside = 0;
                found = 0;
                ...
                if (state.block && (state.block.name === block)) {
                    ...
                    block_start_pos = state.block.sp;
                    block_inside_pos = state.block.ip;
                    block_end_pos = state.block.ep;
                    b_start = state.block.s;
                    b_inside = state.block.i;
                    ...
                    block_end_pos = [line, stream_pos];
                    b_start = stream.sel(pos, stream_pos);
                    b_inside = '';
                    b_end = '';
                    state.block = {
                        name: block,
                ...
                if (found) {
                    ...
                    if (has_interior) {
                        ...
                        if (!already_inside) {
                            ...
                            token.T = type;
                            token.id = block;
                            token.type = modifier || ret;
                            token.str = stream.sel(pos, stream_pos);
                            token.match = null;
                    ...
                    block_inside_pos[1] = [line, stream_pos];
                    block_end_pos = [line, stream.pos];
                    ...
                    if (ended || (!continue_to_next_line && !continued)) {
                        ...
                        state.block.ip = block_inside_pos;
                        state.block.ep = block_end_pos;
                        state.block.i = b_inside;
                        state.block.e = b_end;
                        push_at(state, stack_pos, t_clone(self, is_required, 0, $id));
                    }
                    token.T = type;
            ...
            function t_composite(t, stream, state, token) {
                var self = t,
                    ...
                    tokens = self.token,
                    n = tokens.length,
                    token_izer, style, modifier = self.modifier,
                    found, min, max,
                ...
                if (T_SUBGRAMMAR === type) {
                    self.status &= CLEAR_ERROR;
                    var subgrammar = new String(tokens[0]),
            ...
            var Parser = Class({
                ...
                token: function(stream, state, inner) {
                        ...
                        var self = this,
                            ...
                            $name$ = self.$n$,
                            $type$ = self.$t$,
                            $value$ = self.$v$, //$pos$ = 'pos',
                            interleaved_tokens = grammar.$interleaved,
                            tokens = grammar.$parser,
                            nTokens = tokens.length,
                            niTokens = interleaved_tokens ? interleaved_tokens.length : 0,
                ...
                tokenize: function(stream, mode, row, tokens) {
                        ...
                        if (stream.eol()) { mode.state.line++; if (mode.state.$blank$) mode.state.bline++; } else
                            while (!stream.eol()) tokens.push(mode.parser.get(stream, mode));
                        return tokens;
                    }

                    ,
                parse: function(code, parse_type) {
                        var lines = (code || "").split(newline_re),
                            l = lines.length,
                            linetokens = null,
                            state, mode, parse_errors, parse_tokens, err, ret;
                        ...
                        if (parse_tokens)
                            ...
                            iterate(function(i) {
                                ...
                                if (stream.eol()) { mode.state.line++; if (mode.state.$blank$) mode.state.bline++; } else
                                    while (!stream.eol()) mode.parser.get(stream, mode);
                            }, 0, l - 1);
                        ...
                        if (parse_errors && state.queu /*&& state.queu.length*/ ) {
                            ...
                            while (state.queu /*.length*/ ) {
                                err = state.queu.val /*shift()*/ ;
                                state.queu = state.queu.prev;
                                error_(state, err[1], err[2], err[3], err[4], null, err[5]);
                            }
                        }

                        ret = parse_tokens && parse_errors ? { tokens: linetokens, errors: state.err } :
            ...
            function find_match(dir, iter, row, col, tokenType, S, E, T, folding, commentType) {
                ...
                {
                    var depth = 1,
                        firstLine = iter.first(),
                        i, text, tl, pos,
                        nextOpen, nextClose, row0, col0, Sl = S.length,
                        El = E.length,
                ...
                } else //if ( 1 === dir ) // find end
                {
                    var depth = 1,
                        lastLine = iter.last(),
                        i, text, tl, pos,
                        nextOpen, nextClose, row1, col1, Sl = S.length,
                        El = E.length,
                        unconditional = false === tokenType;
                        ...
                        for (;;) {
                            ...
                            if (!openTag[1] && "autoclosed" !== end) {
                                ...
                                for (;;) {
                                    ...
                                    startLine = iter.row;
                                    startCh = iter.col - (next ? next[0].length : 0);
                                    // found but failed
            ...
            var $CodeMirror$ = 'undefined' !== typeof CodeMirror ? CodeMirror : { Pass: { toString: function() { return "CodeMirror.Pass"; } } },
                ...
                RE_W = /[\w$]/,
                by_score = function(a, b) { return b.score - a.score };
            ...
            var CodeMirrorParser = Class(Parser, {
                ...
                autocomplete: function(cm, options, CodeMirror) {
                        ...
                        var parser = this,
                            ...
                            token, token_i, len, maxlen = 0,
                            word_re, renderer, sort_by_score, score,
                        ...
                        if (dynamic || grammar_tokens) {
                            ...
                            if (!prefix_match)
                                while (end < curLine.length && word_re.test(curLine[CHAR](end))) ++end;
                            token = curLine.slice(start, end);
                            token_i = token[LOWER]();
                            len = token.length;
                                    ...
                                    if (wl >= len) {
                                        ...
                                        if (((pos_i = m1.indexOf(m2)) >= 0) && (!prefix_match || (0 === pos_i))) {
                                            ...
                                            list.push({
                                                ...
                                                name: w,
                                                meta: wm,
                                                start: pos < 0 ? pos_i : pos,
                                                end: (pos < 0 ? pos_i : pos) + token.length,
                                                match: token,
                                                displayText: w + "\t\t[" + wm + "]",
                                                render: renderer,
                                                ...
                                                hint: (cm, data, completion) => {
                                                    ...
                                                    data._handlers = cm._handlers;
                                                    let { name, highlight, from, to } = getTheShowText(completion);
                                                    cm.replaceRange(name, completion.from || data.from,
                                                        completion.to || data.to, "complete");

                                                    if (highlight) {
                                                        let presCursorPosition = cm.getCursor();
                                                        cm.setSelection({ line: presCursorPosition.line, ch: presCursorPosition.ch - 1 }, { line: presCursorPosition.line, ch: (presCursorPosition.ch - (to - from)) }, { scroll: true, bias: -1 })
                                                    }
                                                },
                                            });
                                        }
            ...
            function get_mode(grammar, DEFAULT, CodeMirror) {
                ...
                CMode.options = function(cm, pos, options) {
                    options = options || {};
                    var s = cm.getTokenAt(pos).state,
                        parser = (s && s.parser) || CMode.$parser;
                ...
                CMode.autocompleter = function autocompleter(cm, options) {
                    if (CMode.supportAutoCompletion) {
                        var s = cm.getTokenAt(cm.getCursor()).state,
                            parser = (s && s.parser) || CMode.$parser;
                ...
                CMode.folder = function folder(cm, start) {
                    if (CMode.supportCodeFolding) {
                        var s = cm.getTokenAt(start).state,
                            parser = (s && s.parser) || CMode.$parser,
                            fold;
                    ...
                    if (CMode.supportCodeMatching) {
                        ...
                        setTimeout(function() {
                            cm.operation(function() {
                                ...
                                var options = matcher.options /*|| Mode.matcher.options*/ || {},
                                    maxHighlightLen = options.maxHighlightLineLength || 1000;
                                var marks = [],
                                    ranges = cm.listSelections(),
                                    range,
                                    matched = "CodeMirror-matchingtag" /*"CodeMirror-matchingbracket"*/ ,
                                    unmatched = "CodeMirror-nonmatchingbracket";
...
//~~~~~~~~~codemirror/addon/codemirror_grammar.min.js~~~~~~~~~END~~~~~~~~~~~~~~~~

//~~~~~~~~~expression_editor.js~~~~~~~~~~~START~~~~~~~~~~~~~~~

if (typeof createExpressionEditor === "undefined") {
    (function() {
        // 1. a partial javascript grammar in simple JSON format
        var expression_grammar = {
            ...
            "Parser": [
                ...
                "keyword",
                "operator",
                "atom", ["'}' | ')' | this | identifier | dotProperty", "dotProperty*"]
            ]
        ...
        function codemirror_grammar(code, langs) {
            ...
            for (var i = 0, l = langs.length; i < l; i++) {
                ...
                if (0 === i) {
                    ...
                    main_mode.autocompleter.options = { prefixMatch: true, caseInsensitiveMatch: true, inContext: true, dynamic: true };
            ...
            // use it!
            var autocomplete_cmd = 'autocomplete_grammar_' + main_lang,
                togglecomment_cmd = 'togglecomment_grammar_' + main_lang;
            CodeMirror.commands[autocomplete_cmd] = function(cm) {
                CodeMirror.showHint(cm, main_mode.autocompleter, { completeSingle: false });
            };
            ...
            var opts = {
                ...
                indentUnit: 4,
                indentWithTabs: false,
                extraKeys: { "Ctrl-Space": autocomplete_cmd, "Ctrl-L": "toggleComment","Tab": false,"Shift-Tab": false },
                gutters: ["CodeMirror-lint-markers", "CodeMirror-linenumbers", "CodeMirror-foldgutter"],
                foldGutter: true,
                // enable syntax validation
                lint: true
        ...
        function getTheTokens() {
            if (typeof expression_config !== "undefined") {

                for (key in expression_config) {
                    let presData = expression_config[key];
                    ...
                    expression_grammar.Lex[key] = { "autocomplete": true, tokens: dataToPush };
                }

            } else {
                expression_grammar.Lex.keyword = {
                    "autocomplete": true,
                    "tokens": ["ApprovalNo", "Recordid", "ApprovalStatus", "CompanyName", "UserName", "UserGroup", "UserGroupNo", "UserRoles", "UserCategory", "UserDepartment", "DbRep", "CrRep", "ProfitPath", "AxpertPath", "ConnectionName", "_MainCurr", "_SubCurr", "_currdecimal", "finyrst", "finyred", "afinyrst", "afinyred", "basecurrencyid", "_millions", "_ConnectNo", "SiteNo", "axp_timezone", "axp_language", "axp_datemode"]
                };
                expression_grammar.Lex.function = {
                    "autocomplete": true,
                    "tokens": ["Power", "abs", "IsEmpty", "IsEmptyValue", "Upper", "Lower", "DTOC", "CMonthYear", "CTOD", "MakeDate", "Rnd", "Stuff", "Round", "iif", "AmtWord", "CurrAmtWord", "Val", "Str", "SubStr", "AddToDate", "Date", "Time", "dayofdate", "monthofdate", "yearofdate", "MandY", "DaysElapsed", "TimeElapsed", "GetLength", "AddToTime", "FormatDateTime", "AddToMonth", "LastDayOfMonth", "ValidEncodeDate", "Eval", "Trim", "RegVar", "IsVarEmpty", "sqlget", "findrecord", "firesql", "getdelimitedstr", "extractnum", "trimspace", "findandreplace", "domrp", "mod", "verifytree", "buildtreelink", "convertmd5", "bulkexecute", "constructtable", "posttotable", "savesqlresult", "setdeps", "doStoredProc", "sqlpost", "nowstring", "exportsql", 'postfromiview', "downloadattachment", "AxpCeil", "AxpFloor", "Days360", "NetWorkDays", "SQLRegVar", "encryptstr", "decryptstr", "clonedir", "clonefile", "createfile", "removefile", "xcopydir", "xcopyfile", "deletedir"]
                }
            }

        }

        function getTheShowText(completion) {
            curCMEditorHints.html('');
            let objToSend = {};

            try {
                if (typeof expression_config !== "undefined") {
                    const { meta, name } = completion;
                    let showingText = expression_config[meta][name].selectText;
                     objToSend.name = showingText;
                    if (meta === "function") {
                        objToSend.highlight = true;
                        let openParanthesisIdx = showingText.indexOf("(");
                        let closingParanthesisIdx = showingText.indexOf(")");
                        objToSend.from = openParanthesisIdx
                        objToSend.to = closingParanthesisIdx;
                    }
                ...
                objToSend.name = completion.name;
            }
            objToSend.name = completion.name;
            return objToSend;
        }
        ...
        const createExpressionEditor = function({textarea,customValidationFn}) {
            if (typeof textarea === "string") {
                 textarea = textarea.trim();
                if (textarea.substr(0, 1) === "#") textarea = textarea.substr(1);
                textarea = document.getElementById(textarea);
                $(textarea).parent().append('<span class="expHints"></span>');
            }
            const myEditor = codemirror_grammar(textarea, [
                { language: "expressions", grammar: expression_grammar }
            ]);
            ...
            myEditor.on("focus", function(doc, changeObj) {
                let textArea = $(doc.getTextArea());
                textArea.val(doc.getValue()).focus();
            ...
            myEditor.on('keyup', function(editor, event){
              if (
                ...
                // type code and show autocomplete hint in the meanwhile
                CodeMirror.commands.autocomplete_grammar_expressions(myEditor);
              }
            });
            if (staticRunMode)
                textarea.nextElementSibling.classList.add(..."w-100 border h-100".split(" "));
            else
                textarea.nextElementSibling.classList.add(..."w-100 border min-h-100px h-auto".split(" "));

            $(textarea).data('myeditor', myEditor);

        }

        window.createExpressionEditor = createExpressionEditor;
        window.getTheShowText = getTheShowText;
    })();
}

//~~~~~~~~~expression_editor.js~~~~~~~~~~~END~~~~~~~~~~~~~~~
