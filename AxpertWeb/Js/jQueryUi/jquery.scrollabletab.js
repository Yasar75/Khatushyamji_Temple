...
; (function($) {
    ...
    $.fn.scrollabletab = function(options) {
        ...
        var direction = "left";
        var tabIconPos = "right";
        var previmg = "../AxpImages/icons/16x16/navigate_blright.png";
        var nextimg = "../AxpImages/icons/16x16/navigate_blleft.png";
        var disprevimg = "../AxpImages/icons/16x16/navigate_blright.png";
        var disnextimg = "../AxpImages/icons/16x16/navigate_blleft.png"; 
        if (gl_language == "ARABIC") {
            direction = "right";
            tabIconPos = "left";
            previmg = "../AxpImages/icons/16x16/navigate_blright.png";
            nextimg = "../AxpImages/icons/16x16/navigate_blleft.png";
            disprevimg = "../AxpImages/icons/16x16/navigate_blright.png";
            disnextimg = "../AxpImages/icons/16x16/navigate_blleft.png";
        }
        ...
        return this.each(function() {
            ...
            //Wrap inner items
            if (gl_language == "ARABIC")
                $tabs.wrap('<div id="stTabswrapper" class="stTabsMainWrapper" style="position:relative"/>').find('.ui-tabs-nav').wrapInner('<div class="stTabsInnerWrapper" style="width:30000px"><span class="stWidthChecker"/></div>');
            else
            ...
            var $widthChecker = $tabs.find('.stWidthChecker'),
				...
				$tabsWrapper = $tabs.parents('#stTabswrapper').css("width", "100%");
            ...
            var arrowsTopMargin = (parseInt(parseInt($tabsNav.innerHeight(true) / 2) - 8)),
				arrowsCommonCss = { 'cursor': 'pointer', 'z-index': 10000, 'position': 'absolute', 'top': 3, 'height': $tabsNav.outerHeight() - ($.browser.safari ? 2 : 1) };
            $tabsWrapper.prepend(
			  $nav = $('<div/>')
			  		.disableSelection()
					.css({ 'position': 'relative', 'z-index': 30000, 'display': 'none' })
					.append(
						$('<span/>')
							.disableSelection()
							...
							.css(tabIconPos, 37)
							.append($('<span/>').disableSelection().html('<img id=\"tab-previoustab\" src="' + previmg + '" style="margin-top:5px;"/>').css('margin-top', arrowsTopMargin))
							.click(function() {
							    //Check if disabled
							    if ($(this).hasClass('ui-state-disabled')) return;
							    //Just select the previous tab and trigger scrollToTab event

							    prevIndex = $tabsNav.find('li.ui-tabs-selected').prevAll().filter( ":visible" ).length - 1
							    //Now select the tab
							    $tabsNav.find('li:visible').eq(prevIndex).find('a').trigger('click');
							    return false;
							}),
						$('<span/>')
							.disableSelection()
							...
							.css(tabIconPos, 20)
							.append($('<span/>').html('<img id=\"tab-nexttab\" src="' + nextimg + '" style="margin-top:5px;"/>').css('margin-top', arrowsTopMargin))
							.click(function() {
							    //Just select the previous tab and trigger scrollToTab event
							    nextIndex = $tabsNav.find('li.ui-tabs-selected').prevAll().filter(  ":visible" ).length + 1
							    //Now select the tab
							    $tabsNav.find('li:visible').eq(nextIndex).find('a').trigger('click');
							    return false;
							}),
							$('<span/>')
							.disableSelection()
							...
							.append($('<span/>').html('<img id=\"tab-collapse\" src="../AxpImages/icons/16x16/Collapsebl.png" style="margin-top:5px;"/>').css('margin-top', arrowsTopMargin))

							.click(function() {

							    var tabCntr = GetTabContainer();
							    var tabContrId = "#tabs" + tabCntr + " div";


							    if ($j("#tab-collapse").attr("src") == "../AxpImages/icons/16x16/Collapsebl.png") {
							        $j("#tab-collapse").attr("src", "../AxpImages/icons/16x16/expandbl.png");
							        $j("#tab-collapse").attr("title", "expand");
							        $j(tabContrId).each(function() {
							            if (!$j(this).hasClass("stTabsInnerWrapper"))
							                $j(this).hide();
							        });
							        ...
							        $j("#tab-collapse").attr("src", "../AxpImages/icons/16x16/Collapsebl.png");

							        $j("#tab-collapse").attr("title", "collapse");
							        $j(tabContrId).show();
							        adjustwin('500', window);
							    }
							})
			...
			.bind('navEnabler', function() {
			    setTimeout(function() {
			        ...
			        if (isLast) {
			            ...
			            $pvNav.css('display', 'block');
			            $ntNav.find('img').each(function() {
			                if ($(this).attr("id") == "tab-nexttab")
			                    $(this).attr("src", disnextimg);
			            });
			            $pvNav.find('img').attr("src", previmg);
			        }
			        else if (isFirst) {
			            $ntNav.removeClass('ui-state-disabled');
			            $pvNav.addClass('ui-state-disabled');
			            $ntNav.css('display', 'block');
			            $pvNav.find('img').attr("src", disprevimg);
			            $ntNav.find('img').each(function() {
			                if ($(this).attr("id") == "tab-nexttab")
			...
			.bind('navHandler', function() {
			    ...
			    if ($widthChecker.width() > $tabsNav.width()) {
			        ...
			        if (gl_language == "ARABIC") {
			            $tabsNav.find('li').css("float", "right");
			            $tabsNav.find('li:first').css('margin-right', '5px');
			        } else {
			            $tabsNav.find('li:first').css('margin-left', '5px');
			        }
			    }
			    else {
			        $nav.hide();
