...
(function($, undefined){
    ...
    function _ExecuteQueue(){
        ...
        }).bind("click", _Button_Click).appendTo(buttons);
        ...
        $(window)
            .bind("resize", { messageBox: messageBox }, _Window_Resize)
            .bind("keydown", _Window_KeyDown)
    ...
    function _Button_Click(event){
        ...
        messageBox.animate({
            ...
            $(window)
                .unbind("resize",  _Window_Resize)
                .unbind("keydown", _Window_KeyDown);
            // Remove Overlay
        ...
        if ($this.attr("id") == "messagebox_button_done") {
            queueItem.deferred.resolve(inputValue);
            if ($("#messagebox_content").text() == "Password is changed successfully") {
                LoadParentHome();
            }
        }
    }
