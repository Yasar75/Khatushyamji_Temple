  ...
  return lazypipe().pipe(() => {
    ...
    return rewrite({
      ...
      adaptPath: (ctx) => {
        ...
        // process css only
        if (isCss[0] === ".css") {
          if (/plugins\.bundle/.test(bundle["styles"]) || /datatables\.bundle/.test(bundle["styles"])) {
            const pieces = ctx.sourceDir.split(/\\|\//);
            // only vendors/base pass this
            let vendor = pieces[pieces.indexOf("node_modules") + 1];
            if (/plugins\.bundle/.test(bundle["styles"]) && pieces.indexOf("node_modules") === -1) {
              vendor = pieces[pieces.indexOf("plugins") + 1];
            }

            if (/datatables\.bundle/.test(bundle["styles"]) && pieces.indexOf("node_modules") === -1) {
              vendor = pieces[pieces.indexOf("datatables") + 1];
            }
